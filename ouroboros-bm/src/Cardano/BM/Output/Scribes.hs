...
-- | Create a katip scribe for logging to a handler.
mkFileScribeH :: Handle -> Bool -> Verbosity -> IO Scribe
mkFileScribeH h colorize verbosity = do
    hSetBuffering h LineBuffering
    locklocal <- newMVar ()
    let logger :: Item a -> IO ()
        logger item = --when (checkItem sev sevMap item) $  --TODO probably checkItem is redundant or TraceController should be passed here.
